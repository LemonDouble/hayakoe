...
HayaKoe 는 CPU (ONNX Runtime) 와 GPU (PyTorch) 두 백엔드를 지원합니다. 코드 레벨에서는 `device` 파라미터 하나 차이입니다.
...
- **콜드 스타트가 짧아야 할 때** — ONNX 경로는 프로세스가 뜨는 즉시 `prepare()` 가 끝나고 바로 합성을 받을 수 있습니다. GPU 경로도 기본값은 빠르지만, `compile=True` 를 켰다면 첫 `prepare()` 에서 약 80초의 BERT 컴파일 시간을 감수해야 하므로, 오토스케일·서버리스 환경에서 체감 차이가 큽니다.
...
- **장기 실행 서버 워크로드** — `prepare(compile=True)` 로 BERT 에 `torch.compile` 을 적용하면, 약 80초의 워밍업을 대가로 steady-state 합성이 문장당 약 20% 빨라집니다.

::: details GPU 경로 구성
- **BERT** — FP32 DeBERTa 가 GPU VRAM 에 올라가 임베딩을 계산합니다. 양자화하지 않아 CPU ONNX 경로보다 정밀도가 약간 높습니다. `prepare(compile=True)` 시 `torch.compile` 이 적용되는 유일한 구성 요소입니다.
- **Synthesizer** — PyTorch VITS 디코더 (eager 실행).
- **Duration Predictor** — Synthesizer 와 같은 PyTorch 경로 (eager 실행).
:::

::: tip GPU 백엔드의 콜드 스타트 줄이기
GPU 백엔드의 첫 `prepare()` 는 모델 다운로드가 맞물려 오래 걸릴 수 있고, `compile=True` 를 켰다면 BERT 컴파일 약 80초가 더해집니다. 실제 서비스에서는 아래 두 가지로 이 비용을 미리 치러두는 것이 권장됩니다.
...
- **`prepare(warmup=True, compile=True)`** — prepare 시점에 더미 추론을 선행해 BERT 컴파일과 cuDNN 초기화까지 prepare 로 앞당깁니다. prepare 자체는 조금 더 걸리지만 **첫 실제 요청이 warmup 비용을 떠안지 않게** 됩니다. (→ [FastAPI 통합](/deploy/fastapi))
:::

## 나란히 비교

| 항목 | CPU (ONNX) | GPU (PyTorch) |
|---|---|---|
| 설치 | `pip install hayakoe` | `pip install hayakoe[gpu]` |
| 이미지 크기 | 수백 MB | 수 GB |
| 콜드 스타트 | 빠름 (초) | 빠름 (초) — `compile=True` 시 ~80초 |
| 단일 요청 지연 | 보통 | 최저 |
