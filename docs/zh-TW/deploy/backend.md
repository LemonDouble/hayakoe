...
HayaKoe 支援 CPU (ONNX Runtime) 和 GPU (PyTorch) 兩種後端。
...
- **需要短冷啟動時間時** — ONNX 路徑行程啟動後 `prepare()` 立即完成,可以馬上開始合成。GPU 路徑的預設值同樣很快,但如果開啟 `compile=True`,首次 `prepare()` 需要承受約 80 秒的 BERT 編譯時間,在自動擴縮·無伺服器環境中體感差異很大。
...
- **長期執行的伺服器工作負載** — 透過 `prepare(compile=True)` 對 BERT 套用 `torch.compile`,以約 80 秒的預熱為代價,steady-state 合成速度每句提升約 20%。

::: details GPU 路徑構成
- **BERT** — FP32 DeBERTa 載入到 GPU VRAM 中計算嵌入。因未量化,精度比 CPU ONNX 路徑略高。是 `prepare(compile=True)` 時唯一套用 `torch.compile` 的組件。
- **Synthesizer** — PyTorch VITS 解碼器 (eager 執行)。
- **Duration Predictor** — 與 Synthesizer 相同的 PyTorch 路徑 (eager 執行)。
:::

::: tip 縮短 GPU 後端冷啟動
GPU 後端的首次 `prepare()` 可能因交織模型下載而耗時較久,開啟 `compile=True` 時還會再加上約 80 秒的 BERT 編譯時間。
...
- **`prepare(warmup=True, compile=True)`** — 在 prepare 時預先執行虛擬推論,將 BERT 編譯和 cuDNN 初始化提前到 prepare 階段。prepare 本身更久但 **第一個實際請求不承擔 warmup 成本**。(→ [FastAPI 整合](/zh-TW/deploy/fastapi))
:::
...
| 映像檔大小 | 數百 MB | 數 GB |
| 冷啟動 | 快 (秒級) | 快 (秒級) — `compile=True` 時約 80 秒 |
| 單請求延遲 | 一般 | 最低 |
