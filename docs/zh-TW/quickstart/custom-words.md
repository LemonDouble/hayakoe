...
::: info 已內建約 22 萬個英語單字
HayaKoe 在將文本傳給 pyopenjtalk **之前**,會將英語單字替換為片假名。

pyopenjtalk 原生字典認識的單字(冠詞、縮寫、知名品牌等)直接採用其讀音,只有不認識的單字才透過內部正規化字典替換。

該備援字典預置了 221,591 個英語單字及其對應片假名,因此 `OpenAI`・`GitHub` 等常見英語專有名詞無需呼叫 `add_word()` 就能自然地朗讀。
