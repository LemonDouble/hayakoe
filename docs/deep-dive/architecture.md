...
이 중 **스타일 벡터** 는 `.load(speaker)` 시점에 해당 화자의 `style_vectors.npy` 에서 함께 로드되는 값으로, 화자의 말투 · 톤 특성을 하나의 벡터로 압축한 표상입니다. HayaKoe 는 현재 단순화를 위해 Neutral 스타일만 사용하며 (→ [용어 정리 — Style Vector](./glossary#style-vector)), 매 합성 호출마다 이 값이 그대로 주입됩니다.
