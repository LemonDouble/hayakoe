...
使用 Streaming API (`astream()`) 时每完成一个句子立即 yield,因此可以在全部合成完成前开始播放。在长文本中能 **大幅缩短到首条语音的延迟**(→ [流式传输示例](/zh-CN/quickstart/streaming))。

## 延伸阅读

- **后端选择 (CPU vs GPU) 及 `load` · `prepare` · `generate` 生命周期** — [后端选择](/zh-CN/deploy/backend)
