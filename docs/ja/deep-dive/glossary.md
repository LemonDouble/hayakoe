...
BERT は Synthesizer の **外** に別途存在し全話者が共有します。話者ごとに変わるのは Synthesizer の重み（約 250 MB）です。
...
HayaKoe は GPU 経路で `prepare(compile=True)` を指定すると、共有 BERT に限り `torch.compile` を適用します（デフォルトはオフ）。

約80秒のコンパイル時間と引き換えに1文あたり約20%速くなるため、長時間稼働するサーバーに適しています。

コンパイルコストは `prepare(warmup=True)` を併せて指定すると、サービング開始段階に移すことができます。
