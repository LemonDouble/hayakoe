...
HayaKoe は pyopenjtalk にテキストを渡す **前の** 段階で、英単語をカタカナに置換します。

pyopenjtalk のネイティブ辞書が知っている単語（冠詞・略語・有名ブランドなど）はその読みをそのまま使い、知らない単語だけを内部正規化辞書で置換します。

このフォールバック辞書には 221,591 個の英単語と対応カタカナが事前に含まれているため、`OpenAI`・`GitHub` のような一般的な英語固有名詞は別途 `add_word()` を呼ばなくても自然に読まれます。
