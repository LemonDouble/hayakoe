...
| `noise`            |      `0.6` | 0.3 ~ 0.9   | DP 側に入るノイズ                             |
| `noise_w`          |      `0.8` | 0.5 ~ 1.2   | SDP 側に入るノイズ                            |
...
### `noise` / `noise_w` — 2つの予測器のノイズ

どちらも **長さ予測器** 側に入るノイズの大きさです（音素自体のノイズではありません）。

- `noise` — DP（決定的予測器）に入るノイズ
- `noise_w` — SDP（確率的予測器）に入るノイズ

つまり `sdp_ratio` を 0 にすると `noise_w` はほぼ影響がなく、逆に 1 にすると `noise` がほぼ影響がないという関係です。
