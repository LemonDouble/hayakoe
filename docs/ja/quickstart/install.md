...
GPU モードでは初回の `generate()` 呼び出しが通常より数秒長くかかる場合があります（cuDNN の初期化など）。

2回目の呼び出しからは通常速度が出ます。

サーバーとして起動する場合は、`prepare(warmup=True)` でこのコストを起動段階で先に済ませておくことを推奨します。
:::

::: details さらに速く：BERT torch.compile（オプション）
`prepare(compile=True)` を指定すると、共有 BERT に PyTorch の `torch.compile` を適用します。

`torch.compile` は PyTorch 2.0 で追加された JIT コンパイラで、モデル実行グラフをトレースして一度コンパイルした後、その結果を再利用する方式です。

実測では **約80秒のコンパイルウォームアップ** と引き換えに、steady-state の合成が **1文あたり約20%** 速くなります。

一度コンパイルされたグラフはプロセスが生きている間キャッシュされるため、長時間稼働するサーバーなら有効にする価値があります。

スクリプトや対話的な使用のようにすぐ起動・終了する環境なら、デフォルト（オフ）のままが適しています。
...
CPU（ONNX）モードでは `compile` フラグは無視され、このウォームアップステップは不要です。
:::
