...
HayaKoe 는 GPU 모드일 때 `prepare()` 시점에 PyTorch 의 `torch.compile(mode="reduce-overhead")` 를 자동으로 붙입니다.

`torch.compile` 은 PyTorch 2.0 에서 추가된 JIT 컴파일러로, 모델 실행 그래프를 추적해 한 번 컴파일한 뒤 그 결과를 재사용하는 방식입니다.

덕분에 스트리밍 시나리오에서 프레임당 레이턴시가 눈에 띄게 줄어들지만, 대가로 **첫 호출 때 그래프를 트레이싱·컴파일하는 시간** 이 추가로 듭니다.

HayaKoe 크기의 모델 + `reduce-overhead` 조합에서는 보통 수 초 정도입니다.
