...
HayaKoe 는 pyopenjtalk 에 텍스트를 넘기기 **전** 단계에서 영어 단어를 가타카나로 치환합니다.

pyopenjtalk 네이티브 사전이 아는 단어(관사·약어·유명 브랜드 등)는 그 읽기를 그대로 쓰고, 모르는 단어만 내부 정규화 사전으로 치환합니다.

이 폴백 사전에는 221,591 개의 영어 단어와 대응 가타카나가 미리 들어 있어서, `OpenAI`·`GitHub` 같은 일반적인 영어 고유명사는 따로 `add_word()` 를 호출하지 않아도 자연스럽게 읽힙니다.
