...
→ 实现见 [FastAPI 集成](/zh/deploy/fastapi)
...
→ 实现见 [Docker 镜像](/zh/deploy/docker)

## 板块构成

| 页面 | 内容 |
|---|---|
| [FastAPI 集成](/zh/deploy/fastapi) | lifespan 中加载单例,`agenerate` / `astream`,并发 |
| [Docker 镜像](/zh/deploy/docker) | 构建时 `pre_download`,BuildKit secret,多阶段构建 |
| [后端选择](/zh/deploy/backend) | CPU(ONNX) vs GPU(PyTorch) 权衡 |
