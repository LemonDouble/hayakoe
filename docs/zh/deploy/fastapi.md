...
实测表和复现脚本在 [FAQ — 加载多个说话人时内存增加多少](/faq/#加载多个说话人时内存增加多少) 中整理。
:::
...
→ **解决方法**: 在 Docker 镜像构建时用 `pre_download()` 将权重烘焙到镜像中([Docker 镜像](/deploy/docker) 参考)。运行时的 `prepare()` 完全不走网络。
...
- 烘焙到镜像:[Docker 镜像](/deploy/docker)
- CPU / GPU 选哪个:[后端选择](/deploy/backend)
