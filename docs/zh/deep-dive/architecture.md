...
其中 **风格向量** 是在 `.load(speaker)` 时从该说话人的 `style_vectors.npy` 一同加载的值,是将说话人的说话方式·语气特征压缩为一个向量的表征。HayaKoe 目前为简化仅使用 Neutral 风格(→ [术语表 — Style Vector](./glossary#style-vector)),每次合成调用都直接注入此值。
...
使用 Streaming API (`astream()`) 时每完成一个句子立即 yield,因此可以在全部合成完成前开始播放。在长文本中能 **大幅缩短到首条语音的延迟**(→ [流式传输示例](/zh/quickstart/streaming))。

## 延伸阅读

- **后端选择 (CPU vs GPU) 及 `load` · `prepare` · `generate` 生命周期** — [后端选择](/zh/deploy/backend)
