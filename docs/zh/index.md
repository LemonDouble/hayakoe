...
features:
  ...
  - title: AMD64 · ARM64 全平台
    ...
    link: /quickstart/benchmark#拉-raspberry-pi-4b-实测
    linkText: 树莓派基准测试
  - title: 内存减少 47%
...
想在自己的笔记本上,仅用 CPU 亲自制作上述示例的话,请前往 [10 分钟快速体验](/quickstart/)。
...
1. **首先跟着 [快速开始](/quickstart/)** 走一遍。从安装到首次合成、基准测试,亲自体验这个 TTS 有多快、音质如何。
2. **想要更多的话,前往 [自定义说话人训练](/training/)**。只需一段包含喜欢声音的视频,从数据准备到部署全程指引。
3. **想分享给更多人的话,前往 [服务器部署](/deploy/)**。整理了在 FastAPI · Docker 上以 API 形式发布的方法。
4. **想深入技术细节的话,前往 [深入解读](/deep-dive/)**。逐一解析是如何做到这样的速度和内存优化的。
5. **遇到问题的话,前往 [FAQ](/faq/)**。汇集了缓存路径、Private HF、S3、多说话人内存等高级设置。
