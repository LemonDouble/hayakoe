...
两种后端的差异在 [后端选择](/deploy/backend) 中详细说明。

## 3. 检查点与说话人名称

- 检查点只有 1 个时自动选择,多个时手动选择(通常是在 [③ 质量报告](/training/quality-check) 中选定的)。
...
但运行时侧也需要安装对应后端的依赖。使用 `device="cuda"` 时实际运行的机器上需要安装 **PyTorch CUDA 构建**,`device="cpu"` 仅需基本安装。详情请参考 [安装 — CPU vs GPU](/quickstart/install)。

## 下一步

- 下载使用:[服务器部署](/deploy/)
- 运行时选择哪个后端:[后端选择](/deploy/backend)
