...
from cli.training.dataset import DatasetInfo, discover_datasets, get_model_name
from cli.ui.browser import open_in_browser
...
def _find_checkpoints(dataset_path: Path) -> list[Path]:
    model_name = get_model_name(dataset_path / "config.json")
    if not model_name:
        return []
