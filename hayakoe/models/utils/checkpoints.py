...
def load_checkpoint(
    ...
    """
    지정된 경로에서 체크포인트를 로드하고 모델과 옵티마이저를 업데이트한다.

    Args:
        checkpoint_path (Union[str, Path]): 체크포인트 파일 경로
        model (torch.nn.Module): 업데이트할 모델
        optimizer (Optional[torch.optim.Optimizer]): 업데이트할 옵티마이저. None이면 업데이트하지 않음
        skip_optimizer (bool): 옵티마이저 업데이트를 스킵할지 여부 플래그
...
def clean_checkpoints(
    ...
    """
    지정된 디렉토리에서 오래된 체크포인트를 삭제하여 디스크 공간을 확보한다

    Args:
        model_dir_path (Union[str, Path]): 모델이 저장된 디렉토리 경로
        n_ckpts_to_keep (int): 유지할 체크포인트 수 (G_0.pth, D_0.pth 제외)
        sort_by_time (bool): True이면 시간순으로 삭제, False이면 이름순으로 삭제
    """
...
def get_latest_checkpoint_path(
    ...
    """
    지정된 디렉토리에서 최신 체크포인트 경로를 가져온다

    Args:
        model_dir_path (Union[str, Path]): 모델이 저장된 디렉토리 경로
        regex (str): 체크포인트 파일명의 정규표현식
